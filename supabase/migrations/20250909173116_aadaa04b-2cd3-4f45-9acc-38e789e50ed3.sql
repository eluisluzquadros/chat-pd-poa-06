-- ============================================
-- CORREÇÃO DE FUNÇÕES SECURITY DEFINER - VERSÃO CORRIGIDA
-- Drop e recriar funções para mudar SECURITY mode
-- ============================================

-- 1. REMOVER E RECRIAR FUNÇÕES QUE DEVEM SER SECURITY INVOKER

-- execute_sql_query - DROP e recriar como SECURITY INVOKER
...
-- check_auth_rate_limit - DROP e recriar como SECURITY INVOKER  
...
-- log_user_action - DROP e recriar como SECURITY INVOKER
...
-- handle_new_user - DROP e recriar como SECURITY INVOKER
...
-- track_test_case_changes - DROP e recriar como SECURITY INVOKER
DROP FUNCTION IF EXISTS public.track_test_case_changes();
...
SELECT 
    ...
    CASE 
        WHEN p.proname IN ('get_current_user_role', 'is_admin', 'is_supervisor_or_admin') 
        THEN 'Mantido DEFINER (necessário para RLS)'
        ELSE 'Convertido para INVOKER (mais seguro)'
    END as action_taken
FROM pg_proc p
...
WHERE n.nspname = 'public'
  AND p.proname IN (
    ...
    'get_current_user_role', 'is_admin', 'is_supervisor_or_admin'
  )
ORDER BY 
